...
class Binance {
  ...
  async checkProxyIP(proxy) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new Error(
          `Cannot check the proxy IP. Status code: ${response.status}`
        );
      }
    } catch (error) {
  ...
  async playGameIfTicketsAvailable(
    ...
    console.log(
      `========== Account ${accountIndex} | ${firstName.green} | ip: ${proxyIP} ==========`
    );
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          await this.playGameIfTicketsAvailable(
            queryString,
            i + 1,
            firstName,
            proxy
          ...
          this.log(
            `Error processing account ${i + 1}: ${error.message}`,
            "error"
          );
        }
