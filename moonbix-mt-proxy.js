...
class Binance {
  ...
  getRandomAndroidUserAgent() {
    ...
    return androidUserAgents[
      Math.floor(Math.random() * androidUserAgents.length)
    ];
  }
  ...
  async checkProxyIP() {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new Error(
          `Unable to check proxy IP. Status code: ${response.status}`
        );
      }
    } catch (error) {
  ...
  async startGame(accessToken, axios) {
    try {
      ...
      if (response.data.code === "000000") {
        this.log(
          "Start game successfully, do not close tool before completion",
          "success"
        );
        return true;
      }
  ...
  async playGameIfTicketsAvailable() {
    ...
    let availableTickets =
      userInfo.metaInfo.totalAttempts - userInfo.metaInfo.consumedAttempts;
    ...
    while (availableTickets > 0) {
      this.log(
        `Start the game with ${availableTickets} available tickets`,
        "info"
      );
...
if (isMainThread) {
  ...
  async function runWorkers() {
    while (true) {
        ...
        for (let j = 0; j < maxThreads && i + j < data.length; j++) {
          ...
          const workerPromise = new Promise((resolve, reject) => {
            ...
            worker.on("exit", (code) => {
              if (code !== 0)
                reject(
                  new Error(`The stream is stopped with the code ${code}`)
                );
            });
      ...
      console.log(
        `All accounts processed. Waiting ${
          waitTime / 60000
        } minutes before the next round starts...`
      );
